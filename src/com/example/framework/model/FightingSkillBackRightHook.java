...
public class FightingSkillBackRightHook extends FightingSkillBaseCicks {

	FightingSkillBackRightHook(DynamicGameObject master) {
		super(Statics.FightingSkills.BACK_RIGHT_HOOK, master);
	}
		...
		if(master.objType == Statics.DynamicGameObject.SNAKE) {
			Snake snake = (Snake) master;
		 	...
		 	if(dTime > size *  Statics.Render.SNAKE_GOLD_STRIKE_SEGMENT_CHANGING_PERIOD / 3) {
		 		master.world.world2d.snakeForwardHook(snake.parts.get(size-1), startPos, targetPos, false, maxPath, false);		 		

		 		if(!master.stateHS.striked) {
			 		master.world.world2d.snakeForwardHookImpulse(snake.parts.get(size-1), targetPos, false, maxPath);
